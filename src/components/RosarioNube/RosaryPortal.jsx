...
/**
 * RosaryPortal - Isolated Matter.js rosary with event-based communication
 * Uses CustomEvents and imperative methods to avoid React re-render issues
 */
const RosaryPortal = forwardRef((props, ref) => {
  const sceneRef = useRef(null);
  const matterInstanceRef = useRef(null);

  // Mystery color palettes with gradients
  const getMysteryColors = (mystery) => {
    const palettes = {
      dolorosos: {
        mainLoopStart: "#D2B48C", // Light earth
        mainLoopEnd: "#8B4513", // Dark earth
        tail: "#8B4513",
        cross: "#D2B48C",
      },
      gloriosos: {
        mainLoopStart: "#2F2F2F", // Dark onyx
        mainLoopEnd: "#708090", // Light slate
        tail: "#708090",
        cross: "#2F2F2F",
      },
      gozosos: {
        mainLoopStart: "#FF7F7F", // Coral
        mainLoopEnd: "#FFB6C1", // Light pink
        tail: "#708090",
        cross: "#FF7F7F",
      },
      luminosos: {
        mainLoopStart: "#F5F5DC", // Beige pearl
        mainLoopEnd: "#F8F8FF", // Ghost white
        tail: "#F8F8FF",
        cross: "#F5F5DC",
      },
    };
    return palettes[mystery] || palettes.dolorosos;
  };
  ...
  useImperativeHandle(ref, () => ({
      ...
      allBeads.forEach((bead) => {
        if (bead.isCrossComposite) {
          ...
          bead.crossParts.forEach((part) => {
            const isHighlighted = part.prayerIndex === prayerIndex;
            Matter.Body.set(part, {
              render: {
                fillStyle: isHighlighted ? "#FFD700" : part.originalColor,
                strokeStyle: isHighlighted ? "#FF6B00" : "#94a3b8",
                lineWidth: isHighlighted ? 8 : 1,
              },
            });
          });
        } else {
          // Handle regular beads
          const isHighlighted = bead.prayerIndex === prayerIndex;
          Matter.Body.set(bead, {
            render: {
              fillStyle: isHighlighted ? "#FFD700" : bead.originalColor,
              strokeStyle: isHighlighted ? "#FF6B00" : "#94a3b8",
              lineWidth: isHighlighted ? 8 : 1,
            },
          });
    ...
    updateColors: (mystery) => {
      if (!matterInstanceRef.current) return;

      const colors = getMysteryColors(mystery);
      const { mainLoopBeads, tailBeads, crossParts, heartBead, numMainBeads } =
        matterInstanceRef.current;

      // Update main loop beads with gradient
      const startColorRgb = hexToRgb(colors.mainLoopStart);
      const endColorRgb = hexToRgb(colors.mainLoopEnd);

      mainLoopBeads.forEach((bead, i) => {
        ...
        Matter.Body.set(bead, {
          render: { fillStyle: colorString },
        });
        bead.originalColor = colorString;
      });

      // Update tail beads
      tailBeads.forEach((bead) => {
        Matter.Body.set(bead, {
          render: { fillStyle: colors.tail },
        });
        bead.originalColor = colors.tail;
      });
      ...
      crossParts.forEach((part) => {
        ...
        part.originalColor = colors.cross;
      });

      // Heart bead keeps its white color
      Matter.Body.set(heartBead, {
        render: { fillStyle: "#f1f5f9" },
      });
      heartBead.originalColor = "#f1f5f9";
    },
  }));
  ...
  useEffect(() => {
    ...
    // Default to dolorosos colors
    const colors = getMysteryColors("dolorosos");

    const beadOptions = (color, extraOptions = {}) => ({
      restitution: 0.6, // Reduced from 0.8 for less bounce
      friction: 0.8, // Increased from 0.5 for more friction
      frictionAir: 0.15, // Increased from 0.05 (3x more air resistance)
    ...
    // Create Main Loop Beads with gradient
    const mainLoopBeads = [];
    const startColorRgb = hexToRgb(colors.mainLoopStart);
    const endColorRgb = hexToRgb(colors.mainLoopEnd);
    ...
    // --- Prayer Sequence Mapping ---

    // Cross beads (1-6): Sign of the Cross
    crossParts.forEach((part, idx) => {
      part.prayerIndex = 0;
      part.prayerId = "SC";
      part.beadNumber = idx + 1;
    });

    // Tail beads (7-11): Credo, Contricion, Our Father, Ave Maria, Ave Maria
    tailBeads.forEach((bead, idx) => {
      bead.prayerIndex = 1 + idx;
      bead.prayerId = ["C", "CT", "P", "A", "A"][idx];
      bead.beadNumber = 7 + idx;
    });

    // Heart bead (12): First Mystery
    heartBead.prayerIndex = 6;
    heartBead.prayerId = "M1";
    heartBead.beadNumber = 12;

    // Main loop (13-67): 5 decades (Our Father + 10 Ave Marias each)
    let prayerIdx = 7;
    mainLoopBeads.forEach((bead, idx) => {
      bead.beadNumber = 13 + idx;
      // Every 11th bead is Our Father
      if ((idx + 1) % 11 === 0) {
        bead.prayerId = "P";
      } else {
        bead.prayerId = "A";
      }
      bead.prayerIndex = prayerIdx++;
    });
    ...
    // Store instance for imperative methods
    matterInstanceRef.current = {
      render,
      engine,
      runner,
    ...
    Matter.Events.on(mouseConstraint, "mousedown", (event) => {
      ...
      if (clickedBead) {
        // Fire CustomEvent for app to handle
        const detail = {
          beadNumber: clickedBead.beadNumber,
          prayerIndex: clickedBead.prayerIndex,
          prayerId: clickedBead.prayerId,
        };

        window.dispatchEvent(new CustomEvent("rosary:beadClick", { detail }));
      }
    });
